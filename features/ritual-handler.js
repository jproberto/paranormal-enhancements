...
export async function executeRitual(context) {
    ...
    if (!ritualClassName) {
        warn(`Lógica não encontrada para o slug: ${ritualSlug}`);
...
export async function getRitualClass(slug) {
    ...
    if (!config) {
        warn(`Ritual "${slug}" não possui implementação customizada.`);
        return null;
    }
    ...
    try {
        ...
        error(`Erro ao carregar a classe do ritual "${slug}":`, err);
        return null;
    }
}
